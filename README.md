# Central Bank of Nigeria Exchange Rates API — cbn-exchange-rate
...
The latest Central Bank of Nigeria table is also served keyless, CORS-open and edge-cached, for evaluation, embeds and AI agents:

```bash
curl "https://allratestoday.com/api/open/central-bank/cbn?source=USD&target=NGN"
```

```js
const r = await fetch('https://allratestoday.com/api/open/central-bank/cbn').then((x) => x.json());
...
Requires Node 18+ (global `fetch`); also runs on Bun, Deno and edge runtimes. Also published under the org scope as [`@allratestoday/cbn-exchange-rate`](https://www.npmjs.com/package/@allratestoday/cbn-exchange-rate) — same code, same versions.
...
- Publisher's own page: [Exchange rates](https://www.cbn.gov.ng/rates/) · [www.cbn.gov.ng](https://www.cbn.gov.ng)
- Publication: every business day; the exact schedule, freshness status and any current delay are on the [Central Bank of Nigeria rates page](https://allratestoday.com/central-bank-rates-api/cbn/)
...
- Central Bank of Nigeria quotes **NGN per 1 unit of foreign currency** (e.g. `base: "USD", quote: "NGN"` means NGN per one US dollar).
...
Loading the official Central Bank of Nigeria rate into an accounting system is a supported workflow, not a hack. Step-by-step guides with the direction each system expects:
...
```bash
curl "https://allratestoday.com/api/v1/central-bank/cbn/latest?format=xml&api_key=art_live_..."
```
...
- [Package docs on the site](https://allratestoday.com/docs/sdk/cbn-exchange-rate/) · [ERP integration guides](https://allratestoday.com/docs/integrations/)
